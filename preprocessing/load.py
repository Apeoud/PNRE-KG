...
def init():
    ...
    sentences = [s for s in codecs.open("../data/train.txt", "r", encoding="utf8").readlines()]
    ...
    for mention in train_ht_relation.keys():
        ...
        for tmp in train_head_set[head]:
            if tmp in train_tail_set[tail]:
                if train_ht_relation[entity_mention(head, tmp)] == {"NA"} or train_ht_relation[
                    entity_mention(tmp, tail)] == {"NA"}:
                    continue
